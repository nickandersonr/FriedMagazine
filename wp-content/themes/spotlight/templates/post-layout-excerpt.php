...
<article class="isotopeElement post divider <?php  
		echo 'format-'.$post_data['post_format'] 
			. ' post'.$opt['post_class']
			. (!empty($iso_columns) ? ' '.$iso_columns : '')
			. ($opt['number']%2==0 ? ' even' : ' odd') 
			. ($opt['number']==0 ? ' first' : '') 
			. ($opt['number']==$opt['posts_on_page'] ? ' last' : '')
			. ($opt['add_view_more'] ? ' viewmore' : '')
			. (get_custom_option('show_filters')=='yes' ? ' flt_'.join(' flt_', get_custom_option('filter_taxonomy')=='categories' ? $post_data['post_categories_ids'] : $post_data['post_tags_ids'])
				: '');
		?>">
	<?php
	$size = 'small';
	if (!$post_data['post_protected']) {
		if (!empty($opt['dedicated'])) {
			...
			echo getVideoFrame($post_data['post_video'], $post_data['post_thumb'], false, false, $size);
