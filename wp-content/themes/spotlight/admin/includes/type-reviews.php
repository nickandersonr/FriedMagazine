	...
	function getReviewsMarkup($field, $value, $editable=false, $clear=false, $snippets=false) {
		...
		if (is_array($criterias) && count($criterias) > 0) {
			...
			foreach ($criterias as $num=>$sb) {
				...
				if ($clear || !isset($new_marks[$i]) || $marks[$i]=='' || is_inherit_option($new_marks[$i])) $new_marks[$i] = 0;
				$output .= '<div class="ratBody">'
					. getReviewsSummaryStars($new_marks[$i], $editable)
...
if ( !function_exists( 'getReviewsSummaryStars' ) ) {
	function getReviewsSummaryStars($avg, $editable=false, $snippets=false) {
		...
		if (get_theme_option('reviews_style')=='text') {
			$output .= sprintf($maxLevel<100 ? __('Rating: %s from %s', 'themerex') : __('Rating: %s', 'themerex'), number_format($avg,1).($maxLevel < 100 ? '' : '%'), $maxLevel.($maxLevel < 100 ? '' : '%'));
		} else {
			$stars = str_repeat('<span class="starReview"></span>', 5);
...
if ( !function_exists( 'marksToDisplay' ) ) {
	function marksToDisplay($marks, $max = 0) {
		if(empty($max))
			$maxLevel = max(5, (int) get_custom_option('reviews_max_level'));
		else 
			$maxLevel = $max;
