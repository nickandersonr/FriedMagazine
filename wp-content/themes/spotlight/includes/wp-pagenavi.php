		...
		if($max_page > 1 || intval($pagenavi_options['always_show']) == 1) {
			...
			$output .= '</ul>';
					if ($start_page != $paged && !empty($pagenavi_options['prev_text'])) {
						$output .= '<div class="pager_prev"><a href="'.esc_url(get_pagenum_link($paged-1)).'">'.$pagenavi_options['prev_text'].'</a></div>';
					}
					if ($end_page > $paged && !empty($pagenavi_options['next_text'])) {
						$output .= '<div class="pager_next"><a href="'.esc_url(get_pagenum_link($paged+1)).'">'.$pagenavi_options['next_text'].'</a></div>';
					}	
			$output .= $pagenavi_options['after'];
